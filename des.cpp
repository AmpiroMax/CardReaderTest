...
void IP(WORDC state[], const BYTE in[])
{
    state[0] = BITNUM(in, 57, 31) | BITNUM(in, 49, 30) | BITNUM(in, 41, 29) | BITNUM(in, 33, 28) | BITNUM(in, 25, 27) |
               BITNUM(in, 17, 26) | BITNUM(in, 9, 25) | BITNUM(in, 1, 24) | BITNUM(in, 59, 23) | BITNUM(in, 51, 22) |
               BITNUM(in, 43, 21) | BITNUM(in, 35, 20) | BITNUM(in, 27, 19) | BITNUM(in, 19, 18) | BITNUM(in, 11, 17) |
               BITNUM(in, 3, 16) | BITNUM(in, 61, 15) | BITNUM(in, 53, 14) | BITNUM(in, 45, 13) | BITNUM(in, 37, 12) |
               BITNUM(in, 29, 11) | BITNUM(in, 21, 10) | BITNUM(in, 13, 9) | BITNUM(in, 5, 8) | BITNUM(in, 63, 7) |
               BITNUM(in, 55, 6) | BITNUM(in, 47, 5) | BITNUM(in, 39, 4) | BITNUM(in, 31, 3) | BITNUM(in, 23, 2) |
               BITNUM(in, 15, 1) | BITNUM(in, 7, 0);

    state[1] = BITNUM(in, 56, 31) | BITNUM(in, 48, 30) | BITNUM(in, 40, 29) | BITNUM(in, 32, 28) | BITNUM(in, 24, 27) |
               BITNUM(in, 16, 26) | BITNUM(in, 8, 25) | BITNUM(in, 0, 24) | BITNUM(in, 58, 23) | BITNUM(in, 50, 22) |
               BITNUM(in, 42, 21) | BITNUM(in, 34, 20) | BITNUM(in, 26, 19) | BITNUM(in, 18, 18) | BITNUM(in, 10, 17) |
               BITNUM(in, 2, 16) | BITNUM(in, 60, 15) | BITNUM(in, 52, 14) | BITNUM(in, 44, 13) | BITNUM(in, 36, 12) |
               BITNUM(in, 28, 11) | BITNUM(in, 20, 10) | BITNUM(in, 12, 9) | BITNUM(in, 4, 8) | BITNUM(in, 62, 7) |
               BITNUM(in, 54, 6) | BITNUM(in, 46, 5) | BITNUM(in, 38, 4) | BITNUM(in, 30, 3) | BITNUM(in, 22, 2) |
               BITNUM(in, 14, 1) | BITNUM(in, 6, 0);
}

void InvIP(WORDC state[], BYTE in[])
{
...
WORDC f(WORDC state, const BYTE key[])
{
    BYTE lrgstate[6]; //,i;
    WORDC t1, t2;

    // Expantion Permutation
    t1 = BITNUMINTL(state, 31, 0) | ((state & 0xf0000000) >> 1) | BITNUMINTL(state, 4, 5) | BITNUMINTL(state, 3, 6) |
    ...
    t2 = BITNUMINTL(state, 15, 0) | ((state & 0x0000f000) << 15) | BITNUMINTL(state, 20, 5) | BITNUMINTL(state, 19, 6) |
...
{
    WORDC i, j, to_gen, C, D;
    const WORDC key_rnd_shift[16] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
    const WORDC key_perm_c[28] = {56, 48, 40, 32, 24, 16, 8,  0,  57, 49, 41, 33, 25, 17,
                                  9,  1,  58, 50, 42, 34, 26, 18, 10, 2,  59, 51, 43, 35};
    const WORDC key_perm_d[28] = {62, 54, 46, 38, 30, 22, 14, 6,  61, 53, 45, 37, 29, 21,
                                  13, 5,  60, 52, 44, 36, 28, 20, 12, 4,  27, 19, 11, 3};
    const WORDC key_compression[48] = {13, 16, 10, 23, 0,  4,  2,  27, 14, 5,  20, 9,  22, 18, 11, 3,
...
void three_des3CBC_encrypt(const BYTE in[], BYTE out[], const BYTE key[], size_t in_length, const BYTE iv[])
{
    ...
    BYTE in_buf[DES_BLOCK_SIZE];
    BYTE out_buf[DES_BLOCK_SIZE];
    BYTE ivTmp[DES_BLOCK_SIZE];

    three_des_key_setup(key, three_schedule, DES_ENCRYPT);
    for (int k = 0; k < (int)(in_length / DES_BLOCK_SIZE); k++)
    {
        memcpy(in_buf, in + k * DES_BLOCK_SIZE, DES_BLOCK_SIZE);
        if (k == 0)
            memcpy(ivTmp, iv, DES_BLOCK_SIZE);
        else
            memcpy(ivTmp, out_buf, DES_BLOCK_SIZE);
        memset(out_buf, 0, DES_BLOCK_SIZE);
        for (int i = 0; i < DES_BLOCK_SIZE; i++)
            in_buf[i] = in_buf[i] ^ ivTmp[i];
        three_des_crypt(in_buf, out_buf, three_schedule);
        memcpy(out + k * DES_BLOCK_SIZE, out_buf, DES_BLOCK_SIZE);
    }
}
...
void three_des3CBC_decrypt(const BYTE in[], BYTE out[], const BYTE key[], size_t in_length, const BYTE iv[])
{
    ...
    three_des_key_setup(key, three_schedule, DES_DECRYPT);
    ...
    for (int k = 0; k < (int)(in_length / DES_BLOCK_SIZE); k++)
    {
        ...
        if (k == 0)
            ...
            memcpy(ivTmp, in + (k - 1) * DES_BLOCK_SIZE, DES_BLOCK_SIZE);
        memset(out_buf, 0, DES_BLOCK_SIZE);
        three_des_crypt(in_buf, out_buf, three_schedule);
        for (int i = 0; i < DES_BLOCK_SIZE; i++)
            out_buf[i] = out_buf[i] ^ ivTmp[i];
        memcpy(out + k * DES_BLOCK_SIZE, out_buf, DES_BLOCK_SIZE);
    }
}
...
void bin2hex(const BYTE in[], char out[], size_t in_length)
{
    ...
    for (j = 0; j < (int)in_length; j++)
    {
        out[j * 2] = hexval[((in[j] >> 4) & 0xF)];
        out[(j * 2) + 1] = hexval[(in[j]) & 0x0F];
    }
    out[j * 2] = (char)0;
}
